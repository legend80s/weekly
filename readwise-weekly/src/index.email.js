...
const articles = $articles.map((group) => {
	...
	const subArticles = $subArticles.map((article) => {
		...
		const titleNode =
			article.querySelector("a[class*=titleLink]") ??
		...
		const authorNode = maybeAuthorNode?.querySelector("[class*=docBody]")
			? null
			: maybeAuthorNode;
		...
		return {
			...
			summary: article.querySelector("[class*=docBody]")?.innerText,
			media: article.querySelector("[class*=docSource] span")?.textContent,
